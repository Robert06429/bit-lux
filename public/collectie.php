...
<body class="bg-black">
    ...
    <section class="relative w-full h-[40vh] md:h-[60vh] overflow-hidden">
        <video autoplay muted loop playsinline class="absolute inset-0 w-full h-full object-cover">
            <source src="./images/collectie.mp4" type="video/mp4">
        ...
        <div class="absolute bottom-16 left-16 max-w-xl text-white z-10">
            ...
            <p class="text-sm md:text-base font-light leading-relaxed">
                Discover Bit Lux's exquisite collection of luxury gifts, thoughtfully curated for the discerning woman
                who appreciates timeless elegance and refined craftsmanship.
            </p>
        </div>
    </section>
        ...
        <div class="flex justify-between items-center py-8 px-12">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <?php if (isset($_GET['category'])): ?>
                                ...
                                <?php foreach ($categoryFilters as $cat): ?>
                                    <a href="<?= buildFilterUrl('category', $cat, false) ?>"
                                        class="flex items-center gap-2 px-3 py-1.5 bg-gray-100 text-sm rounded-full hover:bg-gray-200 transition-colors">
                            ...
                            <?php if (isset($_GET['material'])): ?>
                                ...
                                <?php foreach ($materialFilters as $mat): ?>
                                    <a href="<?= buildFilterUrl('material', $mat, false) ?>"
                                        class="flex items-center gap-2 px-3 py-1.5 bg-gray-100 text-sm rounded-full hover:bg-gray-200 transition-colors">
                            ...
                            <?php if (isset($_GET['gender'])): ?>
                                ...
                                <?php foreach ($genderFilters as $gen): ?>
                                    <a href="<?= buildFilterUrl('gender', $gen, false) ?>"
                                        class="flex items-center gap-2 px-3 py-1.5 bg-gray-100 text-sm rounded-full hover:bg-gray-200 transition-colors">
                            ...
                            <?php if (isset($_GET['diamond']) && $_GET['diamond'] == '1'): ?>
                                <a href="<?= buildFilterUrl('diamond', '1', false) ?>"
                                    class="flex items-center gap-2 px-3 py-1.5 bg-gray-100 text-sm rounded-full hover:bg-gray-200 transition-colors">
                            ...
                            <?php if (isset($_GET['price'])): ?>
                                ...
                                <?php foreach ($priceFilters as $price): ?>
                                    ...
                                    elseif ($price == '5000+')
                                        $priceLabel = '$5,000+';
                                    ?>
                                    <a href="<?= buildFilterUrl('price', $price, false) ?>"
                                        class="flex items-center gap-2 px-3 py-1.5 bg-gray-100 text-sm rounded-full hover:bg-gray-200 transition-colors">
                            ...
                            <a href="<?= clearAllFilters() ?>"
                                class="text-xs text-gray-500 hover:text-gray-900 underline transition-colors ml-2">
                                Clear all
                            </a>
                        </div>
                    </div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="relative group">
                    <button
                        class="flex items-center gap-2 px-4 py-2 text-sm font-medium hover:opacity-70 transition-opacity">
                    ...
                    <div
                        class="hidden group-hover:block absolute right-0 top-full mt-2 w-56 bg-white border border-gray-200 shadow-lg z-50 rounded-md overflow-hidden">
                        <?php
                        $sortOptions = [
                            'featured' => 'Featured',
                            'price-asc' => 'Price: Low to High',
                        ...
                        foreach ($sortOptions as $key => $label):
                            ...
                            <a href="?<?= http_build_query($sortParams) ?>"
                                class="block w-full text-left px-4 py-3 text-sm hover:bg-gray-50 transition-colors <?= $sort === $key ? 'bg-gray-50 font-medium' : '' ?>">
                                <?= $label ?>
                            </a>
                        <?php endforeach; ?>
                    </div>
                ...
                <button id="filterBtn"
                    class="flex items-center gap-2 text-sm font-medium hover:opacity-70 transition-opacity">
                    <span>Filters</span>
    ...
    <div id="filterOverlay" class="fixed inset-0 bg-black bg-opacity-50 z-40 hidden transition-opacity duration-300">
    </div>
    ...
    <form method="GET" action="" id="filterForm">
        ...
        <div id="filterSidebar"
            class="fixed top-0 right-0 h-full w-[400px] bg-white z-50 transform translate-x-full transition-transform duration-300 ease-in-out shadow-2xl">
            <div class="flex flex-col h-full">
                ...
                <div class="flex-1 overflow-y-scroll px-8 py-6 scrollbar-hide">
                    ...
                    <div style="margin-bottom: 1rem;">
                        <button type="button"
                            class="flex items-center justify-between w-full py-3 text-left font-medium"
                            onclick="toggleFilter('category')">
                            <span>Category</span>
                            <svg class="w-5 h-5 transform transition-transform" id="categoryArrow" fill="none"
                            ...
                            <?php foreach ($categories as $key => $name): ?>
                                <label class="flex items-center cursor-pointer hover:opacity-70">
                                    <input type="checkbox" name="category[]" value="<?= $key ?>"
                                        class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('category', $key) ? 'checked' : '' ?>>
                    ...
                    <div style="margin-bottom: 1rem;">
                        <button type="button"
                            ...
                            <span>Materials</span>
                            <svg class="w-5 h-5 transform transition-transform" id="materialArrow" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                <polyline points="6 9 12 15 18 9"></polyline>
                            </svg>
                        </button>
                            ...
                            <?php foreach ($materials as $key => $name): ?>
                                <label class="flex items-center cursor-pointer hover:opacity-70">
                                    <input type="checkbox" name="material[]" value="<?= $key ?>"
                                        class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('material', $key) ? 'checked' : '' ?>>
                                    <span class="ml-3 text-sm"><?= $name ?></span>
                                </label>
                            <?php endforeach; ?>
                        </div>
                    ...
                    <div style="margin-bottom: 1rem;">
                        <button type="button"
                            ...
                            <span>Gender</span>
                            <svg class="w-5 h-5 transform transition-transform" id="genderArrow" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                <polyline points="6 9 12 15 18 9"></polyline>
                            </svg>
                        </button>
                        <div id="genderFilter" class="mt-3 space-y-2 hidden">
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="gender[]" value="0"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('gender', '0') ? 'checked' : '' ?>>
                                <span class="ml-3 text-sm">For Her</span>
                            </label>
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="gender[]" value="1"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('gender', '1') ? 'checked' : '' ?>>
                    ...
                    <div style="margin-bottom: 1rem;">
                        <button type="button"
                            class="flex items-center justify-between w-full py-3 text-left font-medium"
                            onclick="toggleFilter('diamond')">
                            <span>Features</span>
                            <svg class="w-5 h-5 transform transition-transform" id="diamondArrow" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                <polyline points="6 9 12 15 18 9"></polyline>
                            </svg>
                        </button>
                        <div id="diamondFilter" class="mt-3 space-y-2 hidden">
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="diamond" value="1"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isset($_GET['diamond']) && $_GET['diamond'] == '1' ? 'checked' : '' ?>>
                    ...
                    <div style="margin-bottom: 1rem;">
                        <button type="button"
                            ...
                            <svg class="w-5 h-5 transform transition-transform" id="priceArrow" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                <polyline points="6 9 12 15 18 9"></polyline>
                            </svg>
                        </button>
                        <div id="priceFilter" class="mt-3 space-y-2 hidden">
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="price[]" value="0-100"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('price', '0-100') ? 'checked' : '' ?>>
                                <span class="ml-3 text-sm">Under $100</span>
                            </label>
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="price[]" value="100-500"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('price', '100-500') ? 'checked' : '' ?>>
                                <span class="ml-3 text-sm">$100 - $500</span>
                            </label>
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="price[]" value="500-1000"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('price', '500-1000') ? 'checked' : '' ?>>
                                <span class="ml-3 text-sm">$500 - $1,000</span>
                            </label>
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="price[]" value="1000-5000"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('price', '1000-5000') ? 'checked' : '' ?>>
                                <span class="ml-3 text-sm">$1,000 - $5,000</span>
                            </label>
                            <label class="flex items-center cursor-pointer hover:opacity-70">
                                <input type="checkbox" name="price[]" value="5000+"
                                    class="w-4 h-4 border-gray-300 accent-black" <?= isFilterActive('price', '5000+') ? 'checked' : '' ?>>
                ...
                <div class="px-8 py-6 border-t border-gray-200">
                    <button type="submit" style="border-radius: 2rem;"
                        class="w-full bg-black text-white py-3 font-medium hover:bg-gray-800 transition-colors mb-3">
                        Show Results
                    </button>
                    <a href="?"
                        class="block w-full text-center text-sm text-gray-500 hover:text-gray-900 transition-colors">
                        Clear All Filters
                    </a>
                </div>
            </div>
        ...
        <?php foreach ($products as $product): ?>
            <div class="group cursor-pointer mb-3">
                <div class="relative aspect-[3/4] lg:aspect-[2/3] overflow-hidden bg-gray-100 mb-3">
                    <button type="button" class="absolute top-4 right-4 z-10 hover:opacity-70 transition-opacity"
                    ...
                    <a href="product.php?id=<?= $product['id']; ?>">
                        <img src="<?= $product["foto1"]; ?>" alt="<?= htmlspecialchars($product["naam"]); ?>"
